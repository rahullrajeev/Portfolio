...
export default function Home() {
  ...
  return (
    <PageTransition>
      ...
      <section ref={showcaseRef} className="py-24 px-6 md:px-12 bg-black text-white overflow-hidden relative">
        <Grid className="max-w-6xl mx-auto items-center relative z-10">
          <div className="col-span-4 md:col-span-4 lg:col-span-6 space-y-6">
            <Reveal direction="up" delay={0.1}>
              <h3 className="font-display text-lg opacity-60 lowercase font-mono">Featured Case Study</h3>
            </Reveal>
            <Reveal direction="up" delay={0.2}>
              <h2 className="font-display text-3xl md:text-4xl font-bold tracking-tighter leading-[0.9]">
                ModuSpark <br /> AI Studio
              </h2>
            </Reveal>
            <Reveal direction="up" delay={0.3}>
              <p className="text-base font-bold font-display opacity-80 max-w-sm">
                AI-powered component generation platform converting natural-language prompts into React + Tailwind code.
              </p>
            </Reveal>
            <Reveal direction="up" delay={0.4}>
              <div className="flex flex-wrap gap-3 group/tags cursor-default transition-opacity duration-300">
                {['Next.js', 'Google Gemini AI', 'Tailwind', 'Monaco Editor'].map(tech => (
                  <span 
                    key={tech} 
                    className="px-4 py-2 rounded-full border border-white/20 text-xs tracking-widest transition-all duration-300 ease-out group-hover/tags:text-transparent hover:!text-white hover:border-white hover:-translate-y-1 transform-gpu"
            ...
            <Reveal direction="up" delay={0.5}>
              <Magnetic strength={15}>
                <Link href="/projects" className="inline-flex items-center gap-2 mt-8 hover:opacity-70 transition-opacity text-sm font-bold tracking-widest">
          ...
          <div className="col-span-4 md:col-span-4 lg:col-span-6 w-full aspect-square md:aspect-[4/5] relative rounded-2xl overflow-hidden bg-zinc-900 border border-white/10 mt-12 md:mt-0">
            <div className="absolute inset-0 flex items-center justify-center font-display opacity-20 rotate-[-10deg] scale-150 text-8xl blur-[4px]">
              MODUSPARK
            </div>
            {/* Minimal mock ui representing the project */}
            <div className="absolute inset-4 md:inset-10 bg-black rounded-xl border border-white/10 shadow-2xl flex flex-col overflow-hidden group">
      ...
      <section className="py-20 px-6 md:px-12 max-w-6xl mx-auto">
        <h2 className="font-display text-2xl md:text-4xl font-bold tracking-tighter mb-12 text-center">
          ...
          {[
            ...
            { title: "Web Design & Development", desc: "Designing and developing responsive, high-converting websites and landing pages tailored for modern web products.", num: "04" },
             ...
             <div key={exp.num} className="col-span-4 md:col-span-4 lg:col-span-6 bg-zinc-50 dark:bg-zinc-950 hover:bg-zinc-900 dark:hover:bg-zinc-50 hover:scale-105 hover:-translate-y-4 hover:shadow-[0_20px_50px_rgba(0,0,0,0.15)] dark:hover:shadow-[0_20px_50px_rgba(255,255,255,0.05)] hover:z-50 z-0 p-10 md:p-12 group transition-all duration-300 ease-out relative cursor-pointer outline outline-1 outline-transparent hover:outline-zinc-200 dark:hover:outline-zinc-800">
                ...
                <h3 className="font-display text-lg md:text-xl font-bold tracking-tight mb-3 text-zinc-900 dark:text-zinc-50 group-hover:text-zinc-50 dark:group-hover:text-zinc-900 transition-colors duration-500">{exp.title}</h3>
                <p className="text-sm md:text-base text-zinc-700 dark:text-zinc-300 font-sans font-light leading-relaxed group-hover:text-zinc-200 dark:group-hover:text-zinc-700 transition-colors duration-500">{exp.desc}</p>
