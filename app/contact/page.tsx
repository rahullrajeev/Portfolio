        ...
        <Grid>
              ...
              <form className="space-y-8" onSubmit={handleSubmit}>
                 <div>
                   <input 
                     ...
                     className="w-full bg-transparent border-b border-zinc-300 py-4 font-mono text-sm tracking-widest focus:outline-none focus:border-black transition-colors"
                   />
                 </div>
                 <div>
                   <input 
                 ...
                 <div>
                   <textarea 
                     ...
                     rows={4}
                     className="w-full bg-transparent border-b border-zinc-300 py-4 font-mono text-sm tracking-widest focus:outline-none focus:border-black transition-colors resize-none"
                   />
                 </div>
                 <button 
                   disabled={isSubmitting || isSuccess}
                   className="group relative overflow-hidden flex items-center justify-between w-full p-6 bg-black dark:bg-zinc-50 border-2 border-black dark:border-zinc-50 text-white dark:text-zinc-900 disabled:bg-zinc-800 disabled:opacity-80 disabled:hover:translate-y-0 disabled:hover:translate-x-0 disabled:hover:shadow-none font-bold tracking-widest text-sm transition-all duration-200 hover:-translate-y-1 hover:-translate-x-1 hover:shadow-[4px_4px_0px_#18181b] dark:hover:shadow-[4px_4px_0px_#fafafa] active:translate-y-0 active:translate-x-0 active:shadow-[1px_1px_0px_#18181b] dark:active:shadow-[1px_1px_0px_#fafafa]"
                 >
                   <span className="relative z-10 flex items-center gap-2">
                     {isSubmitting ? "Transmitting..." : isSuccess ? "Transmission Received" : "Send Transmission"}
                     {isSuccess ? <CheckCircle2 className="w-5 h-5 text-green-400" /> : <Send className="w-5 h-5 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />}
                   </span>
                   <div className="absolute inset-0 bg-zinc-800 translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out z-0"></div>
                   <span className="relative z-10 opacity-0 group-hover:opacity-100 transition-opacity"><Send className="w-5 h-5 translate-x-1 -translate-y-1" /></span>
                 </button>
              </form>
            </div>
          ...
          <div className="col-span-4 md:col-span-4 lg:col-span-6 order-1 md:order-2 flex justify-end">
            <div className="relative w-full h-[300px] md:h-[600px] lg:h-full min-h-[400px] rounded-2xl overflow-hidden mb-8 md:mb-0 mt-2">
               <img src="/monstera.jpeg" alt="Monstera" className="absolute w-full h-full object-cover" />
            </div>
          </div>
        </Grid>
      </section>
